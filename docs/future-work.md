...
| R6 | publication-safety の **synthetic UUID allowlist** を用意し CI を `--strict` 化 | 未着手 | 現状 CI は WARN を失敗にしない（誤失敗回避）。allowlist ができれば UUID も hard-gate 可能 |
| R7 | evaluation artifact の **必須項目チェック**（新 run のモデル・実行日・skill commit）と fixture↔result の **hash 整合** | 未着手 | スキーマ確定後に pure validator へ追加（外部指摘 F2 の残り2項目） |
...
- **対象**: R1、R6、R7、Phase B
- **状態**: in-progress
- **対応**: pure validator 3種を実装（`scripts/check-publication-safety.sh` / `check-markdown-links.sh` / `check-skill-metadata.sh`）し、`.github/workflows/validate.yml`（push/PR、provider なし）で実行。provider/評価は `.github/workflows/manual-empirical.yml`（workflow_dispatch のみ）へ分離。**残り**: evaluation artifact 必須項目チェックと fixture↔result hash 整合（R7）、synthetic UUID allowlist で CI を `--strict` 化（R6）。この2点が入れば resolved。
