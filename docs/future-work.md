...
| R7b | fixture↔result の **hash 整合**（記録後に fixture が改変されていないこと） | 未着手 | manifest 設計が必要。eval 出力は可変なので「fixture 側の固定」を対象にするのが妥当。設計判断待ち |
| R8 | synthetic data boundary の **明示チェック** | 部分的 | 実 session/PII/パスの遮断は publication-safety が実質担保。fixture の synthetic 明示ラベル検証は未実装 |
...
- **状態**: in-progress（大半 resolved、hash 整合 R7b のみ残）
- **対応**: pure validator 4種を実装 — `check-publication-safety.sh`（`--strict` ＋ `.publication-safety-allowlist`）/ `check-markdown-links.sh` / `check-skill-metadata.sh` / `check-eval-artifacts.sh`。`.github/workflows/validate.yml`（push/PR、provider なし）で実行、provider/評価は `.github/workflows/manual-empirical.yml`（workflow_dispatch のみ）へ分離。レビュー推奨の pure CI 6項目のうち **Markdown リンク・skill frontmatter/metadata・evaluation artifact 必須項目・publication safety scan の4つは実装済み**。**残り**: fixture↔result hash 整合（R7b、要 manifest 設計）。synthetic boundary（R8）は publication-safety が実質担保。
